...
def split_by_chars(seq_len, features=None):
    ...
    for i in range(len(headlines)):
        ...
        for i in range(0, len(headline)-seq_len):
            ...
            dataX.append([vocab[char] for char in input].extend(features[i]))
...
def create_summary_by_chars(nepoch, ntrials, seq_len, train=False):
...
if __name__ == "__main__":
    ...
    create_summary_by_words(nepoch, 10, seq_len=50, train=True)
