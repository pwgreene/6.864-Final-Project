...
def run(infile, outfile):
    tweet_processed_text = open(outfile, 'w+').close()
    tweet_processed_text = open(outfile, 'w+')
    fin = open(infile, 'r').readlines()
    for line in fin:
        l = preprocess(line.rstrip())+'\n'
        tweet_processed_text.write(l)
    tweet_processed_text.close()
    print 'hey'
